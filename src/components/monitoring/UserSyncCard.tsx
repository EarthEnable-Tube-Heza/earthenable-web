...
/**
 * UserSyncCard Component
 *
 * Displays user & contact sync job status, configuration,
...
import {
  ...
  useUserSyncHistory,
  useTriggerUserSync,
  useUpdateUserSyncConfig,
} from "@/src/hooks/useMonitoring";
import { StatusIndicator } from "./StatusIndicator";

export function UserSyncCard() {
  const { data: status, isLoading: statusLoading, error: statusError } = useUserSyncStatus();
  const { data: history } = useUserSyncHistory(5);
  const triggerMutation = useTriggerUserSync();
  const updateConfigMutation = useUpdateUserSyncConfig();
    ...
    return (
      <Card className="p-6">
        <div className="text-center text-red-600">
          <p className="font-medium">Failed to load user sync job status</p>
  ...
  const lastDetails = status.last_run?.details;

  return (
    <Card className="p-6">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900">User & Contact Sync</h3>
        <StatusIndicator
      ...
      <div className="grid grid-cols-3 gap-4 mb-4 text-center">
        <div className="bg-gray-50 rounded-lg p-3">
          <p className="text-2xl font-bold text-green-600">{lastDetails?.users_created ?? "-"}</p>
          <p className="text-xs text-gray-500">Users Created</p>
        </div>
        <div className="bg-gray-50 rounded-lg p-3">
          <p className="text-2xl font-bold text-blue-600">{lastDetails?.users_updated ?? "-"}</p>
          <p className="text-xs text-gray-500">Users Updated</p>
        </div>
        <div className="bg-gray-50 rounded-lg p-3">
          <p className="text-2xl font-bold text-gray-900">
            {lastDetails?.employees_created ?? "-"}
          </p>
          <p className="text-xs text-gray-500">Employees Synced</p>
        </div>
      </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="text-gray-500 border-b">
                  ...
                  <th className="text-right py-1 font-medium">Employees</th>
              ...
              <tbody>
                {history.runs.map((run) => (
                  <tr key={run.id} className="border-b border-gray-100">
                    ...
                    <td className="py-2 text-right">{run.details?.users_created ?? "-"}</td>
                    <td className="py-2 text-right">{run.details?.users_updated ?? "-"}</td>
                    <td className="py-2 text-right">{run.details?.employees_created ?? "-"}</td>
